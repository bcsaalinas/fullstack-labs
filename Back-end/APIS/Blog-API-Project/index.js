...
app.get("/posts/:id", (req, res) => {
  ...
  if (targetPost) {
    ...
    res.json({ message: "ERROR, no id matched " });
  }
});
...
app.patch("/posts/:id", (req, res) => {
  ...
  const changeableFields = ["title", "content", "author"];
  let currentDate = new Date();

  if (!targetPost) res.json({ error: "No post found" });

  for (const field of changeableFields) {
    if (req.body[field] !== undefined) {
      targetPost[field] = req.body[field];
    }
  }
  targetPost.date = currentDate;
  res.json(targetPost);
});
